...
def init_db():
    ...
    row = cursor.fetchone()
    if row["count"] == 0:
        # Sem moradores padrão - usuário cadastra durante a apresentação
        moradores_padrao = []
        if moradores_padrao:
            cursor.executemany(
    ...
    # 3. Recorrências - SEM dados padrão (usuário cadastra durante a apresentação)
    # Bloco mantido vazio intencionalmente

    # 4. Transações - SEM dados padrão (app começa vazio para a apresentação)
    # Bloco mantido vazio intencionalmente
